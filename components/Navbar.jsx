...
const  Navbar = () => {
    const [menuOpen, setMenuOpen] = useState(false);  // track navbar state 

  return (
    <div> 
        
        <nav className="bg-gray-400 p-4 gap-3 flex justify-inbetween w-full max-sm:text items-center  ">
            ...
            {navItems.map((items)=>{ 
               ...
               <Link
                ...
                key={items.label}   
                href={items.href}  
                className=" flex items-center justify-between max-sm:hidden "
                >
                {items.label}
